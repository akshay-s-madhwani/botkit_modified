...
module.exports = function beforeinterruptlangdialoglngchfailed(controller) {
  if (controller.plugins.cms) {
    // Initialize variable
    const script = `interruptlangdialog`;
    const thread = `lngchfailed`;

    // fire before onboarding begins
    controller.plugins.cms.before(`interruptlangdialog`, `lngchfailed`, async (convo, bot) => {
      ...
      // map all cms.interruptlangdialog.lngchfailed mustache vars | pattern[0]/*pattern*/ | pattern[1]/*sprintf arguments*/
      ...
      controller.debug(`testing:before:beforeinterruptlangdialoglngchfailed`)(
