...
module.exports = function beforeinterruptquitdialogonTimeout(controller) {
  if (controller.plugins.cms) {
    // Initialize variable
    const script = `interruptquitdialog`;
    const thread = `on_timeout`;

    // fire before onboarding begins
    controller.plugins.cms.before(`interruptquitdialog`, `on_timeout`, async (convo, bot) => {
      ...
      // map all cms.interruptquitdialog.on_timeout mustache vars | pattern[0]/*pattern*/ | pattern[1]/*sprintf arguments*/
      ...
      controller.debug(`testing:before:beforeinterruptquitdialogonTimeout`)(
        `before interruptquitdialog all convo.vars : `,
        convo.vars
      );

      controller.debug(`testing:cms:debug`)(`debug pause interruptquitdialog`);
