...
module.exports = function onchangeinterruptlangdialognewlang(controller) {
  if (controller.plugins.cms) {
    ...
    controller.plugins.cms.onChange(`interruptlangdialog`, `newlang`, async (nv, convo, bot) => {
      ...
      controller.debug(`testing:cms:interruptlangdialog`)(
        `A new value got set for newlang inside interruptlangdialog : `,
        nv
      );

      const i18nvar = [[`str_valuegotset`, { valname: `newlang`, value: nv }]];
      ...
      controller.debug(`testing:onchange:onchangeinterruptlangdialognewlang`)(
        `onchange newlang all convo.vars : `,
        convo.vars
      );
