...
module.exports = function onchangeinterrupthelpdialogsubject(controller) {
  if (controller.plugins.cms) {
    ...
    controller.plugins.cms.onChange(`interrupthelpdialog`, `subject`, async (nv, convo, bot) => {
      // get userState current language
      const loclang = await controller.localizer.getLoclang(controller, bot);

      controller.debug(`testing:cms:interrupthelpdialog`)(
        `A new value got set for subject inside interrupthelpdialog : `,
        nv
      );

      const i18nvar = [[`str_valuegotset`, { valname: `subject`, value: nv }]];
      ...
      controller.debug(`testing:onchange:onchangeinterrupthelpdialogsubject`)(
        `onchange subject all convo.vars : `,
        convo.vars
      );
