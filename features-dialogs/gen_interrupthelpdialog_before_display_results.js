...
module.exports = function beforeinterrupthelpdialogdisplayResults(controller) {
  if (controller.plugins.cms) {
    // Initialize variable
    const script = `interrupthelpdialog`;
    const thread = `display_results`;

    // fire before onboarding begins
    controller.plugins.cms.before(`interrupthelpdialog`, `display_results`, async (convo, bot) => {
      ...
      // map all cms.interrupthelpdialog.display_results mustache vars | pattern[0]/*pattern*/ | pattern[1]/*sprintf arguments*/
      ...
      controller.debug(`testing:before:beforeinterrupthelpdialogdisplayResults`)(
