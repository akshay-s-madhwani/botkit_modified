...
module.exports = function beforeTestHelloonTimeout(controller) {
  if (controller.plugins.cms) {
    // Initialize variable
    const script = `TestHello`;
    const thread = `on_timeout`;

    // fire before onboarding begins
    controller.plugins.cms.before(`TestHello`, `on_timeout`, async (convo, bot) => {
      ...
      // map all cms.TestHello.on_timeout mustache vars | pattern[0]/*pattern*/ | pattern[1]/*sprintf arguments*/
      ...
      controller.debug(`testing:before:beforeTestHelloonTimeout`)(
        `before TestHello all convo.vars : `,
        convo.vars
      );

      controller.debug(`testing:cms:debug`)(`debug pause TestHello`);
