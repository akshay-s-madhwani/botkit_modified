...
module.exports = function onchangehellotemporaryValue(controller) {
  if (controller.plugins.cms) {
    ...
    controller.plugins.cms.onChange(`hello`, `temporary_value`, async (nv, convo, bot) => {
      // get userState current language
      const loclang = await controller.localizer.getLoclang(controller, bot);

      controller.debug(`testing:cms:hello`)(
        `A new value got set for temporary_value inside hello : `,
        nv
      );

      const i18nvar = [[`str_valuegotset`, { valname: `temporary_value`, value: nv }]];
      ...
      controller.debug(`testing:onchange:onchangehellotemporaryValue`)(
        `onchange temporary_value all convo.vars : `,
        convo.vars
      );
