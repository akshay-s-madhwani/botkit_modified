...
module.exports = function beforelangchangeonTimeout(controller) {
  if (controller.plugins.cms) {
    // Initialize variable
    const script = `langchange`;
    const thread = `on_timeout`;

    // fire before onboarding begins
    controller.plugins.cms.before(`langchange`, `on_timeout`, async (convo, bot) => {
      ...
      // map all cms.langchange.on_timeout mustache vars | pattern[0]/*pattern*/ | pattern[1]/*sprintf arguments*/
      ...
      controller.debug(`testing:before:beforelangchangeonTimeout`)(
        `before langchange all convo.vars : `,
        convo.vars
      );

      controller.debug(`testing:cms:debug`)(`debug pause langchange`);
