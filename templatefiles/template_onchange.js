...
module.exports = function replaceFuncName(controller) {
  if (controller.plugins.cms) {
    ...
    controller.plugins.cms.onChange(`replaceScript`, `replaceVariable`, async (nv, convo, bot) => {
      // get userState current language
      const loclang = await controller.localizer.getLoclang(controller, bot);

      controller.debug(`testing:cms:replaceScript`)(
        `A new value got set for replaceVariable inside replaceScript : `,
        nv
      );

      const i18nvar = [[`str_valuegotset`, { valname: `replaceVariable`, value: nv }]];
      ...
      controller.debug(`testing:onchange:replaceFuncName`)(
        `onchange replaceVariable all convo.vars : `,
        convo.vars
      );

      controller.debug(`testing:cms:debug`)(`debug pause replaceScript`);
