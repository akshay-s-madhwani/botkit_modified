...
function generic2emulator(jsonFbGeneric) {
  ...
  const buttonTypeMap = {
    ...
    element_share: { type: `openUrl`, value: `url` },
      ...
      if(jsonFbGeneric && 'elements' in jsonFbGeneric && jsonFbGeneric.elements.length){
    ...
    oaArr[a].script.forEach((thread, b, obArr) => {
      ...
      for (var c = obArr[b].script.length - 1; c >= 0; c--) {
        ...
        if (
          ...
          'elements' in convItem.fb_attachment &&
          ...
          if (
            ...
            ocArr[c].attachmentLayout = `carousel`;
            //const h = { attachmentLayout : `carousel` };
            //ocArr.splice(c+1, 0, ...[...Array(9)].map((_, i) => ocArr[c]));
          }
...
module.exports = async function botkitCmsConfigInit(controller) {
  ...
  await localcms
    ...
    .catch(err => {
      // console.log(`catcherror:catch-${__filename.split(/[/\\]/).pop()}`)(
