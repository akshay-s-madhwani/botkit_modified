 ...
 */
function translateInterpolateDefault(controller, key, ns, lng, defaultValue = undefined) {
  ...
  fse
    ...
    .then(error => {
      ...
      controller.localizer.changeLanguage(lng).then(() => {
        controller.localizer.loadNamespaces([ns]).then(() => {
          const res = controller.localizer.t(key, {
            ns,
            // fallbackLng: [`en`, `fr`],
            lng,
            // lngs: [`en`, `fr`],
 ...
 */
function loadMustacheVars2i18nextFiles(controller, cmsi18ObjArr) {
      ...
      mustacheVars.forEach(mustacheVar => {
        ...
        rawdefs.forEach((rawdef, index) => {
          if (rawdef) {
            ...
            const translated = translateInterpolateDefault(controller, cmsi18FullKey, ns, lng, defaultValue);
            ...
            if (index === rawdefs.length - 1) {
              allInitLang.forEach(il => {
                const translated1 = translateInterpolateDefault(controller, cmsi18FullKey, ns, il, `${il}._${deflngtxt}`);
              });
            }
          }
        });
 ...
 * @returns
 */
function initCmsDialogCustom(controller, results, bfOnAftCodeGenlists) {
  ...
  results.forEach((script, a, oaArr) => {
    ...
    oaArr[a].script.forEach((thread, b, obArr) => {
      ...
      obArr[b].script.forEach((convItem, c, ocArr) => {
        ...
        if (convItem.text && convItem.text[0].indexOf(`typing_`) !== -1) {
  ...
  loadMustacheVars2i18nextFiles(controller, cmsi18ObjArr);
...
module.exports = function botkitCmsConfigInit(controller) {
  ...
  localcms
    ...
    .then(results => {
      ...
      initCmsDialogCustom(controller, results, bfOnAftCodeGenlists);
